...
项目包含 Solidity 合约和中英双语签到页面，使用 TypeScript、Vite 与 ethers v6，无需后端。页面提供简洁的哥特风视觉与中文/英文切换，语言选择保存在浏览器中；中文标题为“签到签到，共同奔向A8！”。电脑版左侧集中每日签到和个人记录，右侧显示排行榜；手机版依次显示签到、记录、排行榜。支持累计签到、当前连续签到、最长连续签到、最近 7 天记录及链上签到排行榜。公开网站只有签到功能，没有合约部署或切换合约入口。

首页人物插画为生成的独立透明 PNG，放在 `public/hero-portrait.png`；展示字体为从 Google Fonts 的 Noto Serif SC 精简出的 WOFF2，`public/fonts/OFL.txt` 保留其 SIL Open Font License。两项资源均随网站静态文件部署，不需要额外图片或字体服务。

钱包可通过 EIP-6963 或注入的 EVM 钱包接口连接，包括 MetaMask、Binance Wallet 等。未连接时显示已发现的钱包选项；连接成功后隐藏，主动断开后再次显示。刷新页面会用钱包已授权的地址自动恢复；点击“断开”后不会自动恢复。
